...
def get_top_videos_data(ad_type=None):
    ...
    query = f"""
        SELECT
            ...
            Adname,
            CTR,
            Descripcion -- Ajustar si tu tabla usa otro nombre
        FROM {bq_client.project}.{DATASET_ID}.{TABLE_NAME}
    """
...
#############################
# 5.1 GENERAR SUGERENCIAS DE CATEGORÍA
#############################
def generate_category_specific_suggestions(videos_data: list, gemini_analysis: dict, ad_type: str) -> list:
    """
    Genera sugerencias adicionales basadas en el historial de la categoría seleccionada,
    aprovechando la información de BigQuery (videos_data) y el análisis del nuevo video.
    """
    category_suggestions = []

    # 1. Agrupar los argumentos más utilizados en la categoría
    freq_args_cat = {}
    for row in videos_data:
        if row.get("commercial_argument"):
            splitted_args = row["commercial_argument"].split(",")
            for arg in splitted_args:
                arg_clean = arg.strip().lower()
                freq_args_cat[arg_clean] = freq_args_cat.get(arg_clean, 0) + 1

    # 2. Identificar los argumentos del nuevo video
    argumentos_nuevos = gemini_analysis.get("argumentos_comerciales", [])
    argumentos_nuevos_lower = [a.lower() for a in argumentos_nuevos]

    # 3. Tomar los argumentos top y sugerir los que falten
    top_args_cat = sorted(freq_args_cat.items(), key=lambda x: x[1], reverse=True)[:5]
    for arg, freq in top_args_cat:
        if arg not in argumentos_nuevos_lower:
            category_suggestions.append(
                f"En la categoría '{ad_type}', el argumento comercial '{arg}' "
                f"ha aparecido con frecuencia (usado en {freq} anuncios exitosos). Considera incluirlo."
            )
        ...
        if st.button("Interpretar JSON en lenguaje natural"):
            if "gemini_result" not in st.session_state:
                ...
                with st.spinner("Generando interpretación en lenguaje natural..."):
                    try:
                        interpretation = analyze_video_with_gemini(prompt=prompt_interpretacion).get(
                            "raw_response", 
                        ...
                        if successful_videos_info and isinstance(gemini_result, dict) and "analisis" in gemini_result:
                            suggestions, estimated_ctr = generate_suggestions(
                                gemini_result["analisis"], 
                                successful_videos_info, 
                                avg_ctr
                            )

                            st.markdown(f"""
                                <div class="section-title">Sugerencias de Mejora (Generales):</div>
                            ...
                            # Generar sugerencias específicas para la categoría
                            category_suggestions = generate_category_specific_suggestions(
                                successful_videos_info,
                                gemini_result["analisis"],
                                selected_ad_type
                            )
                            ...
                            if category_suggestions:
                                st.markdown(f"""
                                    <div class="section-title">Sugerencias Específicas para la Categoría '{selected_ad_type}':</div>
                                    <ul class="suggestion-list">
                                        {"".join(f"<li>{cs}</li>" for cs in category_suggestions)}
                                    </ul>
                                """, unsafe_allow_html=True)
                            else:
                                st.info(f"No se encontraron sugerencias específicas para la categoría '{selected_ad_type}'.")

                        else:
                            # Si no hay datos o no se pudo parsear
                            st.info("")
