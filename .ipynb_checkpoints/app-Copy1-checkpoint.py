...
def get_average_ctr():
    """
    Devuelve la media de la columna CTR. Lanza error si la dataset/tabla no existen.
    """
...
def get_top_videos_data(threshold_multiplier=1.0):
    """
    Retorna registros de los videos que superan (threshold_multiplier * promedio CTR).
    Ajusta los campos y la lógica según tu tabla real.
    """
...
#############################warning
# 3. FUNCIÓN ANALIZAR VIDEO CON GEMINI
#############################
def analyze_video_with_gemini(local_file_path: str, prompt: str) -> dict:
    """
    Sube el video en 'local_file_path' a la File API de Gemini,
    espera estado ACTIVE y hace un generate_content con el 'prompt'.
    Devuelve un dict parseado de la respuesta JSON.
    """
    # 1. Subir archivo
    ...
    # 3. Llamar al modelo con el prompt
    model = genai.GenerativeModel(model_name=DEFAULT_MODEL_NAME)
    response = model.generate_content(
        [file_info, prompt],
        request_options={"timeout": 600}  # 10 minutos, ajusta según necesidad
    )

    # 4. Parsear la respuesta como JSON
    try:
        # Limpiar la respuesta de caracteres Markdown
...
#############################
def main():
    st.title("Analizador de Videos con Gemini + BigQuery")

    st.markdown("""
    **Objetivos**:
    1. Analizar video con la API de Gemini (música, emociones, etc.).
    2. Comparar con patrones de éxito (CTR) en BigQuery.
    3. Generar sugerencias de mejora.
    4. Estimar CTR potencial.
    """)

    # 5.1 Verificar CTR promedio
    try:
        avg_ctr = get_average_ctr()
    ...
    if avg_ctr is None:
        st.warning("No se pudo obtener CTR promedio. Posiblemente no hay datos en la tabla.")
        avg_ctr = 0.2  # fallback

    st.write(f"**CTR promedio de la categoría**: {avg_ctr:.2f}%")

    # 5.2 Obtener videos "exitosos"
    try:
        successful_videos_info = get_top_videos_data(threshold_multiplier=1.0)
    except Exception as e:
        st.error(f"Ocurrió un error obteniendo datos de videos exitosos: {e}")
        successful_videos_info = []

    # 5.3 Subir video: local o drive
    ...
    # 5.4 Análisis con Gemini
    # Verificamos si el archivo está creado (local)
    if os.path.exists(temp_video_path):
        if st.button("Analizar con Gemini"):
            # Prompt
            prompt_analisis = """
Analiza este video publicitario de Facebook Ads. Realiza un análisis cualitativo exhaustivo, siguiendo estrictamente el siguiente formato de respuesta en JSON. Los timestamps deben estar en formato "MM:SS".
...
Proporciona el análisis siguiendo estrictamente este formato JSON. La consistencia del formato es crítica. Todos los campos deben ir llenados. Los campos que correspondan a listas deben ser llenados con objetos, si no hay datos, deben ir vacíos.

}

Después de proporcionar el JSON anterior, analiza el orden de aparición de los siguientes argumentos comerciales. Para cada uno que aparezca, indica el timestamp (MM:SS) y el contexto específico de su aparición. Utiliza el siguiente formato:

ANÁLISIS DE ARGUMENTOS COMERCIALES POR ORDEN DE APARICIÓN:

1. [timestamp] - [Argumento] - [Contexto/Forma en que se presenta]
2. [timestamp] - [Argumento] - [Contexto/Forma en que se presenta]
(etc.)

Lista de argumentos a identificar:
- Salud y Bienestar
- Diseño y Moderno
...
[Lista de los argumentos que no aparecieron en el video]

Nota: Solo incluir en la numeración los argumentos que efectivamente aparecen en el video, en el orden exacto de su aparición.
            """
            with st.spinner("Analizando con Gemini..."):
                try:
                    gemini_result = analyze_video_with_gemini(temp_video_path, prompt_analisis)
                except Exception as e:
                    st.error(f"Error analizando el video en Gemini: {e}")
                    st.stop()

            st.subheader("Resultado del Análisis")
            st.json(gemini_result)

            # 5.5 Generar sugerencias
            suggestions, estimated_ctr = generate_suggestions(
                gemini_analysis=gemini_result,
                videos_data=successful_videos_info,
                avg_ctr=avg_ctr
            )
            ...
            if suggestions:
                ...
                st.write("No se generaron sugerencias específicas.")

            # 5.6 Mostrar CTR estimado
            st.subheader("Estimación de CTR Potencial")
            st.write(f"CTR estimado para este video: **{estimated_ctr:.2f}%**")
